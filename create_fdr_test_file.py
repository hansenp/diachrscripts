...
i_count = 0
p_treshs = arange(p_value_step, p_value_max + p_value_step, p_value_step)
print(p_treshs)
for p_tresh in p_treshs:
    i_count_range = 0
    for d_inter in interaction_set.interaction_list:
        if (p_tresh - p_value_step < d_inter.get_pval()) and d_inter.get_pval() <= p_tresh:
            i_count_range  += 1
    ...
    if i_count_range < i_count_per_range:
        print("[WARNING] Could not select the required number (only "
              ...
              + str(p_tresh - p_value_step) + ';' + str(str(p_tresh)) + ']')
    print(str(p_tresh - p_value_step) + '\t' + str(p_tresh) + '\t' + str(i_count_range)+ '\t' + str(i_count))
