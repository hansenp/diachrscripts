...
class kInteractionCounter:
    ...
    def get_expected_number_zero_interactions(self, k):
        p = 2 * (0.5 ** k)
        print p
        print (self.k_interaction_dict[k][1] + self.k_interaction_dict[k][2])
        return p * (self.k_interaction_dict[k][1] + self.k_interaction_dict[k][2])
    ...
    def print_k_interaction_counts(self):
        ...
        for i in range(2, self.max_k + 1):
            ...
            n_zero_total_expected = str(round(self.get_expected_number_zero_interactions(i)))
            print(k + "\t" + n_zero_simple + "\t" + n_zero_twisted\
                  + "\t" + n_zero_total\
                  + "\t" + f_zero_total\
...
k_interaction_counter = kInteractionCounter(10)
...
with gzip.open(diachromatic_interaction_file, 'r' + 't') as fp:
    ...
    while line:
        ...
        if not(interaction.is_cis_long_range(10001)):
