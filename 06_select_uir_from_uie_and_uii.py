...
##################################################
####X The regions of the enriched digests from the Javierre publication are stored to a set.
...
########################
####X The following files will be generated by the script.
...
#########################################################################
####X Here the distribution of read pairs per interaction is determined for directed interactions (DI).
...
with gzip.open(enhanced_interaction_file, 'rt') as fp:
    ...
    for line in fp:
        ...
        ####X Here we use the enriched digest regions from the Javierre publication in order to determine the enrichment status of digests involved in this iteraction.
        ...
        if interaction_category == 'DI':
            ...
            if enrichment_pair_tag == 'EE':
                ...
                ####X We count directed interactions with certain numbers of read pairs using dictionaries.
                ...
                ####X We use different dictionaries for EE, EN, NE and NN, because we assume that the enrichment status has an influence on the distribution of read pairs per interaction.
        ...
        ####X We keep track of the read pair numbers per interaction for directed (DI) all undirected interactions (UI) during the first pass.
        ####X During the second pass, we will do the same for undirected reference interactions (UIR).
...
####X This case occured at some point and I added this. But actually that never happens.
...
######################################################
####X During the second pass, we use the dictionaries with the interaction counts for the various per interaction read pair counts in order to select undirected reference interactions.
...
with gzip.open(enhanced_interaction_file, 'rt') as fp:
    ...
    for line in fp:
        ...
        # Get enrichment status of digests from file for enriched digests
        ####X As in the first pass, we use the enriched digest regions of the Javierre publication in order to determine the enrichment status of interactions.
        if coord_key_da in enriched_digests_set:
            enrichment_tag_dig_1 = 'E'
        else:
            enrichment_tag_dig_1 = 'N'
        ...
        if interaction_category == 'UIE' or interaction_category == 'UII':
            ...
            if enrichment_pair_tag == 'EE' and rp_total in di_ee_rp_dict and 0 < di_ee_rp_dict[rp_total]:
                ####X Whenever we select an undirected reference interaction with a certain number of read pairs, we decrement the corresponding value in the dictionary (interaction count) by 1.
                di_ee_rp_dict[rp_total] -= 1
        ...
        ####X We overwrite the enrichment pair tag in column 6 of the EI file with the new tag that results from Javierre's file.
        ####X Up until now there were tags here that resulted from GOPHER's  short-cut option 'All protein coding genes'.
        ####X We are now using EE, EN, NE or NN instead of AA, AI, IA or II.
        line = diachrscripts_toolkit.set_column_in_enhanced_interaction_line(line, 6, enrichment_pair_tag)
        ####X We overwrite the interaction category tag in column 3 of the EI file with either DI, UI or UIR.
        line = diachrscripts_toolkit.set_column_in_enhanced_interaction_line(line, 3, interaction_category_tag)
        ####X We write all input interactions to a new EI file with modified tags for interaction category and enrichment status.
        di_ui_uir_enhanced_interaction_stream_output.write(line + "\n")
        if interaction_category_tag == 'DI' or interaction_category_tag == 'UIR':
            ####X In a second EI file, we write DI and UIR interactions only. This results in a much smaller file that is better suited for some analyzes.
...
####X For directed interactions with high read pair counts, there is sometimes no corresponding reference interaction.
####X In such cases the corresponding count in the dictionary is not zero.
####X Such cases will be reported in the file for warnings.
...
##########################
####X During the selection of the reference interaction, we determined the distributions of read pair counts per interaction for DI, UI and UIR.
####X With this we can check afterwards whether the distributions for DI and UIR are really the same and how they differ from the distribution for UI.
...
####X The most important results are written to the screen.
print()
...
####X All results are written to a table.
...
#########################################
####X We create boxplots for the distributions of read pairs per interaction.
...
############################################################################
####X We create four barplots (for EE, EN, NE and NN) for the numbers and proportions of interactions in DI, UI and UIR.
