...
do
  ...
  md5sum $OUT_DIR/$SRR\_1.fastq.gz >> $OUT_DIR/md5.txt
  # Reverse
  ...
  md5sum $OUT_DIR/$SRR\_2.fastq.gz >> $OUT_DIR/md5.txt
done
# Compress concatenated files
gzip $OUT_DIR/$OUT_PREFIX\_1.fastq # Forward
echo $OUT_PREFIX >> $OUT_DIR/md5.txt
md5sum $OUT_DIR/$OUT_PREFIX\_1.fastq >> $OUT_DIR/md5.txt
gzip $OUT_DIR/$OUT_PREFIX\_2.fastq # Reverse
md5sum $OUT_DIR/$OUT_PREFIX\_2.fastq >> $OUT_DIR/md5.txt
