...
class DiachromaticParser:
    ...
    """
    def __init__(self, interaction_file_dir:str, required_replicates: int, prefix: str) -> None:
        self._dir = interaction_file_dir
        if not os.path.isdir(self._dir):
            raise FileNotFoundError("Could not find directory at ", self._dir)
        self._gzfiles = []
        self._required_replicates = required_replicates
        self._prefix = prefix
        for file in os.listdir(self._dir):
            if file.endswith(".tsv.gz"):
                gzpath = os.path.join(self._dir, file)
        ...
        print("[INFO] We found %d Diachromatic interaction files at %s" % (len(self._gzfiles), self._dir))
