...
if digest_black_list != None:
    with open(digest_black_list, 'rt') as fp:
        for line in fp:
            chrom, sta, end = line.rstrip('\n').split('\t')
            digest_to_be_excluded.add(chrom + '\t' + str(sta) + '\t' + str(end))
## TODO -- Why do we split and then join back again? Don't we need to remove the new line character?
...
eiParser = EnhancedInteractionParser(enhanced_interaction_file)
enhanced_interactions = eiParser.parse()

for ei in enhanced_interactions:
    coord_key_da = ei.coordinate_key_a
    coord_key_db = ei.coordinate_key_b
    # Add associated digests to sets
    if ei.interaction_category in directed_interaction_category_tags:
